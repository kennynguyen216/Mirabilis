    ...
    const glm::vec3& previousPosition)
{
    ...
    constexpr float PlaneContactEpsilon = 0.01f;
    constexpr float MinimumEntrySpeed = 0.01f;
    const float entrySpeed = glm::dot(_playerMovement.velocity, source.normal);

    const float forwardMove = (_playerInput.forward ? 1.0f : 0.0f) -
        (_playerInput.backward ? 1.0f : 0.0f);
    const float rightMove = (_playerInput.right ? 1.0f : 0.0f) -
        (_playerInput.left ? 1.0f : 0.0f);
    const glm::vec3 inputForward{
        std::sin(_playerInput.yaw), 0.0f, -std::cos(_playerInput.yaw)};
    const glm::vec3 inputRight{
        std::cos(_playerInput.yaw), 0.0f, std::sin(_playerInput.yaw)};
    glm::vec3 inputDirection = inputForward * forwardMove + inputRight * rightMove;
    if (glm::dot(inputDirection, inputDirection) > 0.0f) {
        inputDirection = glm::normalize(inputDirection);
    }
    const float entryIntent = glm::dot(inputDirection, source.normal);

    const bool movingThroughPlane = entrySpeed < -MinimumEntrySpeed;
    const bool pressingIntoPortal = entryIntent < -0.05f;
    const float playerCenterDistance = portal_signed_distance(
        source, _playerMovement.position);
    const bool flushWithPortalWall = std::abs(
        playerCenterDistance - playerHalfWidth) <= 0.05f;
    const glm::vec3 tangentVelocity = _playerMovement.velocity -
        source.normal * entrySpeed;
    const bool grindingAlongWall =
        glm::dot(tangentVelocity, tangentVelocity) > 0.01f;
    const bool crossedPortalPlane =
        previousDistance >= -PlaneContactEpsilon &&
        currentDistance <= PlaneContactEpsilon &&
        previousDistance - currentDistance > 0.000001f;

    // Wall resolution often leaves the leading face exactly on the wall. At
    // shallow approach angles it can also remove all normal velocity, leaving
    // the player sliding exactly along the plane. In that contact case, input
    // directed into the portal opens the wall instead of requiring a crossing
    // that collision resolution has made impossible.
    const bool enteringFromFlushContact = flushWithPortalWall &&
        (pressingIntoPortal || grindingAlongWall);
    if ((!movingThroughPlane || !crossedPortalPlane) &&
        !enteringFromFlushContact) {
        return false;
    }
    ...
    const float crossingFraction = distanceDelta > 0.000001f
        ...
        : 1.0f;
    const glm::vec3 crossingLeadingFace = enteringFromFlushContact
        ? currentLeadingFace
        : glm::mix(previousLeadingFace, currentLeadingFace, crossingFraction);
    ...
    // Move the collision box fully onto the room side of the exit wall while
    // preserving any distance already carried through the transform.
    constexpr float ExitEpsilon = 0.02f;
    const float requiredExitDistance = playerHalfWidth + ExitEpsilon;
    const float currentExitDistance = portal_signed_distance(
        destination, _playerMovement.position);
    if (currentExitDistance < requiredExitDistance) {
        _playerMovement.position += destination.normal *
            (requiredExitDistance - currentExitDistance);
    }
